...
func _spawn_player(peer_id: int) -> void:
	if spawn_points.is_empty():
		return
	var spawn_transform := _pick_spawn_transform()
...
func _spawn_bots() -> void:
	...
	for i in range(bot_count):
		var spawn_transform := _pick_spawn_transform()
		rpc("spawn_bot", next_bot_id, spawn_transform)
		next_bot_id += 1

@rpc("authority", "reliable", "call_local")
func spawn_bot(bot_id: int, spawn_transform: Transform3D) -> void:
...
func _pick_spawn_transform() -> Transform3D:
	...
	var marker := spawn_points[rng.randi_range(0, spawn_points.size() - 1)]
